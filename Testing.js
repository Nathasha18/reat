...
export default class Testing extends Component {
    ...
    componentDidMount() {
        ...
        return fetch('http://192.168.43.233/view.php')
    ...
    UpdateDataToServer = (index) => {
        ...
        if (this.state.data[index].Type.length == 0 || this.state.data[index].Amount.length == 0 || this.state.data[index].Description.length == 0 || this.state.data[index].OrderID.length == 0 || this.state.data[index].ServicePersonID.length == 0) {
            alert("required field is Missing");
        } else {
            fetch('http://192.168.43.233/Update2.php', {
                method: 'POST',
                headers: {
                ...
                body: JSON.stringify({
                    ...
                    id: this.state.data[index].ExtraChargeID,

                    type: this.state.data[index].Type,

                    amount: this.state.data[index].Amount,

                    description: this.state.data[index].Description,

                    orderID: this.state.data[index].OrderID,

                    servicePersonID: this.state.data[index].ServicePersonID,
    ...
    DeleteDataToServer = (index) => {
        ...
        // const date = new Date()
            ...
            fetch('http://192.168.43.233/delete.php', {
                method: 'POST',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json',
                ...
                <FlatList
                    ...
                    renderItem={({ item, index }) =>
                        ...
                        <View style={{ backgroundColor: "#D5D8DC", padding: 10, margin: 10 }}>
                            ...
                            <Text>Amount</Text>
                            <TextInput
                                onChangeText={text => this.onChangeHandler({ key: 'Amount', value: text }, index)}
                                value={item.Amount}
                                style={styles.inputs} />
                            ...
                            <TextInput
                                ...
                                value={item.OrderID}
                                style={styles.inputs}/>
                            <Text>ServicePersonID</Text>
                            <TextInput
                               onChangeText={text => this.onChangeHandler({ key: 'ServicePersonID', value: text }, index)}
                                value={item.ServicePersonID}
                                style={styles.inputs}/>
